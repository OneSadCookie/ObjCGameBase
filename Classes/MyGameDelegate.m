...
- (void)gameDidLoad
{
    image = [[GBImage alloc] initWithURL:[NSURL fileURLWithPath:@"/Library/Desktop Pictures/Classic Aqua Blue.jpg"]];
    if (!image) abort();
...
- (void)drawAtSize:(NSSize)size
{
    ...
    float xOffset = 1.0f / (((float)size.width) / ((float)[image size].width));
    float yOffset = 1.0f / (((float)size.height) / ((float)[image size].height));
